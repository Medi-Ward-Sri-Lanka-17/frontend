...
export const retrieveWardDetailsMore = async (wardNo) => {
  try {
    const response = await apiClient.get(`wards/more/${wardNo}`)
    return response.data
  } catch (error) {
    if (error.response) {
      console.error('Error Response:', error.response.data)
...
export const updateWardDetailsMore = async (wardNo, value) => {
  try {
    const response = await apiClient.put(`/ward/edit/admin/${wardNo}`, value)
    showSuccessAlert(response.data)
  } catch (error) {
    if (error.response) {
      console.error('Error Response:', error.response.data)
